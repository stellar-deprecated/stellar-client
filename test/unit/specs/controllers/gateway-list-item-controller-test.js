...
describe('Controller: GatewayListItemCtrl', function () {
  ...
  it('You should be able to remove the current gateway', function () {
    ...
    expect(inner_session.get('wallet').mainData.gateways['removing-gateway'].domain).to.equal(scope.gateway.domain);
    scope.remove();
    expect(scope.gateway.status).to.equal('removing')
    expect(inner_session.get('wallet').mainData.gateways['removing-gateway'].status).to.equal('removing');
    rootScope.$apply();
    expect(inner_session.get('wallet').mainData.gateways['removing-gateway']).to.equal(undefined);
  ...
  it('You should be able to cancel adding a gateway', function () {
    ...
    expect(inner_session.get('wallet').mainData.gateways['test-gateway']).to.exists;
    scope.cancelAdd();
    expect(inner_session.get('wallet').mainData.gateways['test-gateway']).to.be.undefined;
  });
