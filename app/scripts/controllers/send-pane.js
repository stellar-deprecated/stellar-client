...
sc.controller('SendPaneCtrl', ['$rootScope','$scope', '$routeParams', '$timeout','session','stNetwork', 'rpFederation', 'rpReverseFederation', 'rpTracker', function($rootScope, $scope, $routeParams, $timeout, session, $network, $federation, $reverseFederation, $rpTracker )
{
    ...
    $scope.$watch('send.recipient', function (newVal, oldVal) {
        ...
        $scope.contact = null; //TODO: will we have contacts also? webutil.getContact($scope.userBlob.data.contacts, address);
    ...
    $scope.check_destination = function () {
        ...
        if (!stellar.UInt160.is_valid(recipient)) return;
        ...
        account.entry(function (e, data) {
            $scope.$apply(function () {
                ...
                if (e) {
                    ...
                    send.xtr_deficiency = reserve_base.subtract(data.account_data.Balance);
    ...
    $scope.reset_amount_deps = function () {
        var send = $scope.send;
        send.sender_insufficient_xtr = false;
        send.quote = false;
    ...
    $scope.update_amount = function () {
        ...
        if (!stellar.Amount.is_valid(amount) || amount.is_negative()) {
            ...
            $scope.sendForm.send_amount.$error['rpAmount'] = false;
        }
        ...
        if (send.quote_url) {
            ...
            if (total.compareTo(reserve_base) < 0) {
                send.fund_status = "insufficient-xtr";
                send.xtr_deficiency = reserve_base.subtract(send.recipient_info.Balance);
            }
            ...
            $.ajax({
                ...
                success: function (data) {
                    $scope.$apply(function () {
                        ...
                        if (!data || !data.quote || !(data.result === "success" || data.status === "success") || !Array.isArray(data.quote.send) || !data.quote.send.length || !data.quote.address) {
                            $scope.send.path_status = "error-quote";
    ...
    $scope.update_paths = function () {
        ...
        if (send.pathfind &&
            send.pathfind.src_account === $rootScope.account.Account &&
            send.pathfind.dst_account === recipient &&
            send.pathfind.dst_amount.equals(amount))
            return;

        // Start path find
        var pf = $network.remote.path_find($rootScope.account.Account,
            recipient,
            amount);
                ...
                if (!tracked) {
                    $rpTracker.track('Send pathfind', {
                        ...
                        'Address Type': $scope.send.bitcoin ? 'bitcoin' :
                            $scope.send.federation ? 'federation' : 'stellar',
    ...
    */
    ...
    $scope.send_confirmed = function () {
        ...
        if ($scope.send.quote) {
            ...
            tx.payment($rootScope.account.Account,
